...
const animateScreenshots = () => {
  ...
  const screenshots = section.querySelectorAll(".animated-screenshot");

  if (window.scrollY + window.innerHeight > section.offsetTop + 100) {
    screenshots.forEach((item, index) => {
      setTimeout(() => {
        item.classList.add("visible");
      }, index * 200); // з затримкою, щоб по черзі
    });

    window.removeEventListener("scroll", animateScreenshots); // лише один раз
  }
};

window.addEventListener("scroll", animateScreenshots);
